        ...
        <div className="mt-3 text-center">
          ...
          <div className="mt-2 px-7 py-3">
            <p className="text-sm text-gray-500">
              Are you sure you want to delete {collections.length} collection{collections.length > 1 ? 's' : ''}?
            </p>
            ...
            <div className="mt-4 max-h-32 overflow-y-auto">
              <h4 className="text-sm font-medium text-gray-700">Collections to delete:</h4>
              <ul className="text-left text-sm text-gray-600 mt-1">
                {collections.map((collection) => (
                  <li key={collection.id} className="py-1">• {collection.name}</li>
                ))}
              </ul>
            </div>
          </div>
...
export default function Collections() {
  ...
  const calculateCascadeInfo = (selectedCollectionIds: number[]) => {
    ...
    selectedCollectionObjects.forEach(collection => {
      ...
      // Note: We don't have API key permission count in the frontend data
      // The backend will provide this info in the response
    });
  ...
  const handleDeleteClick = () => {
    ...
    const cascade = calculateCascadeInfo(selectedIds);
    setCascadeInfo(cascade);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to delete collections';
          ...
          {filteredCollections.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredCollections.map((collection) => {
                ...
                >
                  {/* Checkbox - positioned to avoid card click area */}
                  ...
                  <div className="flex items-center space-x-3 ml-8">
                    ...
                    <div className="flex-1 min-w-0">
                      {/* Make only the title clickable, not the entire card */}
                      <Link 
                        to={`/collections/${collection.id}`} 
                        className="block hover:text-blue-600 focus:outline-none"
                      >
                        <p className="text-lg font-medium text-gray-900 truncate hover:text-blue-600 transition-colors">
                          {collection.name}
                        </p>
                      </Link>
                      <p className="text-sm text-gray-500">
