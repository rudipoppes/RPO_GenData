...
export default function ApiKeys() {
  ...
  const loadApiKeys = async () => {
    try {
      ...
      // Load collection info for each API key
      const collections: {[key: number]: any[]} = {};
      for (const key of data) {
        try {
          const keyCollections = await apiKeysApi.getAllowedCollections(key.id);
          collections[key.id] = keyCollections;
        } catch (err) {
          console.warn(`Failed to load collections for API key ${key.id}`, err);
          collections[key.id] = [];
        }
      }
      setKeyCollections(collections);
    } catch (err: any) {
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="mt-1 text-sm text-gray-500">
                        ...
                        <div>
                          Collections: {keyCollections[apiKey.id]?.length === 0 ? 'All Collections' : 
                            keyCollections[apiKey.id]?.map(c => c.collection_name).join(', ') || 'Loading...'}
                        </div>
